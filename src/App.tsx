  ...
  return (
    <div className="min-h-screen relative" style={{background:"#282828",fontFamily:"Inter,'Segoe UI',system-ui,sans-serif"}}>
      ...
      <div className="flex items-center gap-1.5" style={{height:'env(titlebar-area-height, 32px)',background:"#191919",borderBottom:"1px solid #232323",WebkitAppRegion:'drag' as any,appRegion:'drag' as any,position:'fixed',top:'env(titlebar-area-y, 0)',left:'env(titlebar-area-x, 0)',width:'env(titlebar-area-width, 100%)',zIndex:30,paddingLeft:8,paddingRight:8,boxSizing:'border-box'}}>
        ...
        {'documentPictureInPicture' in window && <svg onClick={togglePip} width={14} height={14} viewBox="0 0 16 16" className="shrink-0 cursor-pointer" style={{WebkitAppRegion:'no-drag' as any,opacity:pipWin?.9:.5}} title="Pin on top"><rect x="1" y="3" width="10" height="10" rx="1" fill="none" stroke={pipWin?"#7BAEFA":"#7A7A7A"} strokeWidth="1.2"/><rect x="5" y="1" width="10" height="10" rx="1" fill={pipWin?"#7BAEFA":"#383838"} stroke={pipWin?"#7BAEFA":"#7A7A7A"} strokeWidth="1.2"/><path d="M8 4.5v4M6 6.5h4" stroke={pipWin?"#383838":"#7A7A7A"} strokeWidth="1" strokeLinecap="round"/></svg>}
      ...
      <div style={{background:"#303030"}}>
        ...
        <div className="flex justify-end" style={{padding:"4px 6px"}}>
          <div onClick={()=>{setAddOpen(!addOpen);haptic('light');}} className="flex items-center gap-0.5 cursor-pointer" style={{height:18,padding:"0 4px",background:"#383838",border:"1px solid #303030",borderRadius:3}}><svg width={12} height={12} viewBox="0 0 16 16"><path d="M8 3v10M3 8h10" stroke="#D2D2D2" strokeWidth="1.4" strokeLinecap="round"/></svg><svg width={7} height={7} viewBox="0 0 16 16" style={{transform:addOpen?"rotate(180deg)":"",transition:"transform .15s"}}><path d="M3 5h10L8 11z" fill="#7A7A7A"/></svg></div>
        </div>
